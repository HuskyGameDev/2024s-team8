...
const lines: Array[String] = [
	"A thick layer of vines prevents anyone from accessing the command deck...",
]
...
func _ready():
	if !PositionManager.HasRemovedCommandDeckVines:
...
func _on_interact():
	...
	if PositionManager.HasShears:
		...
		await DialogManager.dialog_finished
		queue_free()
		PositionManager.HasRemovedCommandDeckVines = true
